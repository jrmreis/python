def main(r,h,c):
    ...
    if (n > m):
        if (n % (m+1) == 0):
            print("\nVocê começa!\n")
            usuario_escolhe_jogada(n,m,r,h,c)
        else:
            print("\nComputador começa!\n")
            computador_escolhe_jogada(n,m,r,h,c)
    else:
        print("\nOps! Houve um erro de digitação!\n")
        main(r,h,c)
        
def computador_escolhe_jogada(n,m,r,h,c):
        if ((n - m) > 0):
            n = n - m
            print("O computador tirou", m ,"peça(s).\nAgora restam",n,"peças no tabuleiro\n")
            return usuario_escolhe_jogada(n,m,r,h,c)
        else:
...
def usuario_escolhe_jogada(n,m,r,h,c):
    n = jogada(n,m)
    if (( n - m ) > 0) or (m != 0):
        return computador_escolhe_jogada(n,m,r,h,c)
    else:
